	...
	try {
		...
		function initOptions() {
			try {
				...
				// Task definition
				ApiOptions = getApiOptions(ApiName, ApiTask);
				ApiParams  = ApiOptions.param == null ? [] : ApiOptions.param
				...
				var missing = [];
				var params  = {};
				var opts = nopt(args, shortHands, process.argv, 2);
				[CliOptions, ApiOptions].forEach(function(d) {
					d.forEach(function(p) {
						if (opts[p.name] != null)
							params[p.name] = opts[p.name];
						else if (p.optional === false)
							missing.push(p);
					});
				})
				if (missing.length > 0) {
